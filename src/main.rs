...
fn deploy_path(path: &str, signing_key: &str, target_host: &str,
               use_substitutes: bool, use_local_sudo: bool) -> io::Result<()> {
...
fn deploy_system(path: &str, target_host: &str, use_remote_sudo: bool,
                 action: &str, profile: &str) -> io::Result<()> {
    ...
    let profile_path = match action {
        "switch" | "boot" | "reboot" => if profile == "system" {
    ...
    let remote_action = if action == "reboot" {
        "boot"
    } else {
        action
    };
    run_cmd!("{} {}/bin/switch-to-configuration {}", ssh_tool, profile_path, remote_action)?;
    if action == "reboot" {
    ...
    let matches = App::new(crate_name!())
        ...
        .subcommand(
            SubCommand::with_name("path")
                ...
                .arg(Arg::with_name("signing-key")
                    ...
                    .required(true))
                .arg(Arg::with_name("use-substitutes")
                    .short("s")
                    .long("use-substitutes")
                    .help("Attempt to download missing paths on the target \
                ...
                .arg(Arg::with_name("use-local-sudo")
                    .long("use-local-sudo")
                    .help("When set, nix-simple-deploy prefixes local commands \
        ...
        .subcommand(
            SubCommand::with_name("system")
                ...
                .arg(Arg::with_name("use-remote-sudo")
                    .long("use-remote-sudo")
                    .help("When set, nix-simple-deploy prefixes remote commands \
    ...
    match matches.subcommand() {
        ("path",  Some(path_matches)) => {
            deploy_path(
                ...
                path_matches.is_present("use-local-sudo")).unwrap();
        },
        ("system", Some(system_matches)) => {
            deploy_path(
                ...
                system_matches.is_present("use-local-sudo")).unwrap();
            deploy_system(
                system_matches.value_of("PATH").unwrap(),
                system_matches.value_of("target-host").unwrap(),
                system_matches.is_present("use-remote-sudo"),
                system_matches.value_of("ACTION").unwrap(),
                system_matches.value_of("profile").unwrap()).unwrap();
        },
        _ => unreachable!(),
    }
}
